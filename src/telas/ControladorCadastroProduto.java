...
public class ControladorCadastroProduto implements Initializable {
	...
	private Produto getFormData() {
		...
		if (txtDescProd.getText() == null || txtDescProd.getText().trim().equals("")) { // trim = Para eliminar qualquer
																						// espaço em branco no inicio ou
																						// no final.
			excecao.addErros("descricao", " O campo não pode estar vazio!");
		}
		prod.setDescProd(txtDescProd.getText().toUpperCase());
		
		if (txtPreco.getText() == null || txtPreco.getText().trim().equals("")) { // trim = Para eliminar qualquer espaço em branco no inicio ou no final.
			excecao.addErros("preco", " O campo não pode estar vazio!");
		}
