...
public class ControladorProduto implements Initializable, AtualizaDadosLista {
	...
	private ServicoProduto service; // Injetar a prodeência sem colocar a implementação 'new ServicoProduto'
	...
	public void onBtNewAction(ActionEvent event) { // Ação que ocorrerá após o botão 'Novo' ser clicado -- (ActionEvent
		...
		cadastroDialogoFormulario(prod, "/telas/ProdutoCadastro.fxml", parentStage); // Chamada do método de formulário de
																					// cadastro
	}
	...
	private void InitializeNodes() {
		...
		colunaCodigo.setCellValueFactory(new PropertyValueFactory<>("codigo")); // Comando para iniciar apropriadamento
																				// o
		// comportamento da coluna na tabela
...
//																											// Diálogo
		try {
			...
			ControladorCadastroProduto controller = loader.getController();
			controller.setProduto(prod);
			controller.setServicos(new ServicoProduto(), new ServicoGrupoProduto());
			...
			dialogoStage.setTitle("Informe os dados do Produto");
	...
	private void inicBotaoEditar() {
		...
		colunaEditar.setCellFactory(param -> new TableCell<Produto, Produto>() {
			...
			protected void updateItem(Produto obj, boolean empty) {
				...
				setGraphic(button);
				button.setOnAction(
						event -> cadastroDialogoFormulario(obj, "/telas/ProdutoCadastro.fxml", Utils.currentStage(event)));
			}
		});
	}
